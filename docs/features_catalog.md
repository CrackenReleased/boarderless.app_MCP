...
Successful MCP create, mutate, delete, group, ungroup, reorder, undo, and redo calls refresh `<board-name>--<board-id>.bdrl.json` in the configured workspace. Agents should confirm the workspace before composing and explicitly call `export_board_file` before handoff.
