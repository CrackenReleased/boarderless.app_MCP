...
Status: bundle `dist/boarderless-mcp-0.1.27.mcpb` built and smoke-tested 2026-07-03.
...
1. `manifest.json` (MCPB manifest_version 0.3) at repo root — declares all 20 tools, `tools_generated: true`, `privacy_policies: ["https://boarderless.app/privacy"]`, and `user_config` for workspace folder, canvas URL, and debugging address (mapped to `BOARDERLESS_WORKSPACE_DIR`, `BOARDERLESS_MCP_APP_URL`, `BOARDERLESS_MCP_BROWSER_URL`).
...
5. `icon.png` (256×256, from `src-tauri/icons/128x128@2x.png`). Claude Desktop recommends 512×512 — swap in a larger asset when available.
...
- Bundle: 21.5 MB packed / 45.4 MB unpacked, sha1 `809e627c9e0a1781ba65fbfd4c2e9bb027e087c7`.
...
- [ ] Create a **reviewer test account** on boarderless.app (Google account or Free Mode path) with a pre-populated board. Reviewers must reach a working canvas without guesswork.
...
> 3. Sign in on the canvas page: [TEST ACCOUNT EMAIL / CREDENTIALS HERE — or use the local Free Mode fallback if Google sign-in is unavailable].
