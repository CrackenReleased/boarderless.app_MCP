...
async function main() {
  ...
  console.log(`Connecting to debugging Chrome at ${browserURL}...`);
  
  const browser = await puppeteer.connect({ browserURL });
  const pages = await browser.pages();
  
  // Locate the Boarderless tab
  const appOrigin = 'https://boarderless.app';
  const page = pages.find(p => p.url().startsWith(appOrigin));
  
  if (!page) {
    console.error('[-] Error: Boarderless canvas page not found in Chrome. Please make sure the tab is open.');
  ...
  if (!isAuthenticated) {
    ...
    console.log('Please log in using Google OAuth in the opened Chrome browser window.');
  ...
  // Create the new board and switch to it
  ...
  console.log(`[+] New board created successfully with ID: ${boardId}`);
  await new Promise(r => setTimeout(r, 1500)); // wait for canvas hydration

  console.log('[*] Seeding geometric whimsical pattern...');
  ...
  if (drawResult.success) {
    console.log('[+] Whimsical T-shirt Pattern layout generated successfully on your Boarderless canvas!');
  } else {
