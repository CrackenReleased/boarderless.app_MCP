...
VS Code / GitHub Copilot can use Boarderless as a local stdio MCP server through `.vscode/mcp.json`. OpenAI ChatGPT connectors and Microsoft Copilot Studio generally require future hosted HTTPS/OAuth adapter work rather than the current local stdio server. See [docs/connector_distribution_plan.md](docs/connector_distribution_plan.md) for config examples and readiness status; see [docs/connector_operator_runbook.md](docs/connector_operator_runbook.md) for Joel's operator gates and safe validation commands.
